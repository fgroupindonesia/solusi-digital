...
<body>
    ...
    <tbody>
        <?php if (count($data_orders) > 0): ?>                 
            <?php foreach($data_orders as $key): ?>
                <tr>
                    ...
                    <td class="text-truncate"><i class="fas fa-eye"></i>
                        <a class="detail-order-link" href=""  data-ordertype="<?= htmlspecialchars($key->order_type) ?>"
                           data-orderid="<?= htmlspecialchars($key->order_id) ?>"
                           data-bs-toggle="modal"
                           data-bs-target="#detail-order-form-modal"> View </a>
                    </td>

                     <td class="text-truncate">
                        <?php if (strtolower($key->status) != 'cancel' && strtolower($key->status) != 'pending') :?>
                        <i class="fas fa-download"></i> <a class="invoice-order-link" href="" data-order-client-reff="<?= htmlspecialchars($key->order_client_reff) ?>"> Download </a>
                    <?php endif;?>
                    </td>
                    <td>
                         <?php if (strtolower($key->status) == 'completed' || strtolower($key->status) == 'revision' || strtolower($key->status) == 'delivered' ) :?>
                           <i class="fas fa-key"></i> <a data-id="<?= $key->order_id; ?>" data-order-type="<?= $key->order_type; ?>" class="client-end-result-link" data-bs-target="#detail-order-result-view-modal" data-bs-toggle="modal" href="#"> Access </a>
                        <?php endif;?>
                    </td>
        ...
        <?php endif; ?>
                ...
                <?php include('modal_detail_order_end_result_view_only.php'); ?>
                <?php include('modal_revisions_client.php'); ?>
